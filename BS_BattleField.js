...
export class BattleField {
    ...
    calculate_damage(caster, target, rate, dmgType, skillObj = null, status = null) {
        ...
        const F = getHeishuFactor(caster.unit_type,target.unit_type);
        ...
        for (const state of statesToProcess) {
            if (state.name === "反撃") {
                if(Math.random() <= (state.rate /100)){
                    ...
                    this.add_log(`  !!${actor.colored_name} の反撃不発！`);
                }
            }
            ...
            if (isContDmg) {
                const dmgRate = Math.min(parseInt(state.rate) ,100);
    ...
    handleSpecialStates(actor, phase, side, target, state) {
        ...
        if(state.name === "風林火山-風"){
            /**風林火山-風の処理*/
            if(this.turn >= Number(state.value2) && this.turn <= Number(state.value2) +1 ){
                logMsg = ` -> ${actor.colored_name} が 風旗を掲揚`;
                this.add_log(logMsg);
                const newTargets = this.find_targets(actor, "ally_random_2-3", state.source_skill.type);
                for (let newTarget of newTargets){
                    const val = Math.round(((state.value -100) * 0 + 22) * 100) /100
                    const effect = { type: "buff_stat", value: val, stat: "dmg_up_weapon", duration: 1 ,conflict_rule: "EXTEND"};
                    state.source_skill.add_buff(actor, newTarget, effect, this);
                }
            }
        }
        if(state.name === "風林火山-林"){
            /**風林火山-林の処理*/
            if(this.turn >= Number(state.value2) && this.turn <= Number(state.value2) +1 ){
                logMsg = ` -> ${actor.colored_name} が 林旗を掲揚`;
                this.add_log(logMsg);
                const newTargets = this.find_targets(actor, "enemy_random_2-3", state.source_skill.type);
                for (let newTarget of newTargets){
                    const val = Math.round(((state.value -100) * 0 + 92) * 100) /100
                    this.process_attack_event(state.source_busho, newTarget, val, "intel", state.source_skill);
                }
            }
        }
        if(state.name === "風林火山-火"){
            /**風林火山-火の処理*/
            if(this.turn >= Number(state.value2) && this.turn <= Number(state.value2) +1 ){
                logMsg = ` -> ${actor.colored_name} が 火旗を掲揚`;
                this.add_log(logMsg);
                const newTargets = this.find_targets(actor, "enemy_random_1", state.source_skill.type);
                for (let newTarget of newTargets){
                    const val = Math.round(((state.value -100) * 0 + 156) * 100) /100
                    const atkcnt = Math.floor(Math.random() * 2) + 1;
                    for(let i =0; i < atkcnt; i++) {
                        this.process_attack_event(state.source_busho, newTarget, val, "weapon", state.source_skill);
                    }
                }
            }
        }
        if(state.name === "風林火山-山"){
            /**風林火山-山の処理*/
            if(this.turn >= Number(state.value2) && this.turn <= Number(state.value2) +1 ){
                logMsg = ` -> ${actor.colored_name} が 山旗を掲揚`;
                this.add_log(logMsg);
                const newTargets = this.find_targets(actor, "ally_random_2-3", state.source_skill.type);
                for (let newTarget of newTargets){
                    const val = Math.round(((state.value -100) * 0 + 22) * 100) /100
                    const effect = { type: "buff_stat", value: val, stat: "dmg_cut_weapon", duration: 1 ,conflict_rule: "EXTEND"};
                    state.source_skill.add_buff(actor, newTarget, effect, this);
                }
            }
        }
        ...
        if(state.name === "槍の又左-鉄壁(予備)"){
            /**槍の又左-鉄壁の処理*/
            const has_teppeki = actor.states.find(s => s.name === '鉄壁' && s.source_skill == "槍の又左");
            if(has_teppeki){
                has_teppeki.value = 2;
                const logMsg = ` -> ${state.source_busho.colored_name} が ${actor.colored_name} に ${has_teppeki.name}(${has_teppeki.value}) を付与 (${has_teppeki.duration}ターン)`;
                this.add_log(logMsg);
            }else{
                const newState = {
                    name: "鉄壁",
                ...
                const isSuccess = actor.add_state(newState, this);
                if (isSuccess) {
                    const logMsg = ` -> ${state.source_busho.colored_name} が ${actor.colored_name} に ${newState.name}(${newState.value}) を付与 (${newState.duration}ターン)`;
                    this.add_log(logMsg);
                }
            }
            const has_kyouka = actor.states.find(s => s.name === '槍の又左-強化(予備)');
            has_kyouka.value = Number(has_kyouka.value) + 1;
        }
        
        if(state.name === "槍の又左-強化(予備)"){
            /**槍の又左-強化の処理*/
            const val = state.source_busho.is_main ? 70 : 50;
            if (state.value ==2){
                //通常攻撃UP付与
                const effect1 = { type: "buff_stat", value: val, stat: "dmg_up_normal", duration: 1, clear:true };
                state.source_skill.add_buff(state.source_busho, actor, effect1, this);
                //
                const newState1 = {
                    type: "special",
                ...
                const isSuccess = actor.add_state(newState1, this);
                if (isSuccess) {
                    const logMsg = ` -> ${state.source_busho.colored_name} が ${actor.colored_name} に ${newState1.name} を付与 (${newState1.duration}ターン)`;
                    this.add_log(logMsg);
                }
                state.value = 0;
        ...
        if(state.name === "槍の又左(予備)"){
            /**槍の又左の処理*/
            let newTargets = this.get_enemies(actor);
            newTargets = newTargets.filter(b => b.name != target.name && b.hp > 0);
            const atk_val = 100 + actor.current_dmg_up_normal;
            for(let newTarget of newTargets){
                this.process_attack_event(state.source_busho, newTarget, atk_val, "weapon", state.source_skill);
            }
            ...
            actor.states.forEach(s => {
                if (s.name =="槍の又左(予備)"){
                    ...
                    if (idx > -1) actor.states.splice(idx, 1);
                    logMsg = ` -> ${actor.colored_name} の 槍の又左(予備) が 消失 `;
                    this.add_log(logMsg);
                }
            })
            actor.states.forEach(s => {
                if (s.source_skill.name =="槍の又左" && s.stat == "dmg_up_normal"){
                    const statName = STAT_MAP[s.stat];
                    const val = s.value;
                    idx = actor.states.indexOf(s);
                    if (idx > -1) actor.states.splice(idx, 1);
                    logMsg = ` -> ${actor.colored_name} の ${statName} が ${val} 減少 (現在: ${actor.current_dmg_up_normal})  `;
                    this.add_log(logMsg);
                }
            })
        }
        ...
        if(state.name === "武田之赤備(予備)"){
            /**武田之赤備の処理*/
            const rate = 25 + actor.kaishin ? 25 :0 + actor.is_main ? 10 :0;
            if (Math.random() <= rate /100){
                const targetKey = "enemy_random_1";
                const newTargets = this.find_targets(actor, targetKey, state.source_skill.type);
                this.process_attack_event(actor, newTargets[0], 138, "weapon", state.source_skill);
                ...
                this.add_log(`   ${actor.colored_name} は【${state.source_skill.colored_name}(予備)】発動失敗！`);
            }
        }

        if(state.name === "武田之赤備-威勢(予備)"){
            /**武田之赤備-威勢の処理*/
            const effect = { type: "debuff_stat", value: "15%", stat: "ldr", duration: 2};
            state.source_skill.add_buff(actor, target, effect, this);
        }

        if(state.name === "諸行無常(予備)"){
            /**諸行無常の処理*/
            if(this.turn >= 4 && this.turn <= 6){
                const effect1 = { type: "debuff_stat", value: state.value, stat: "dmg_up_weapon", duration: 1};
                state.source_skill.add_buff(actor, actor, effect1, this);
                const effect2 = { type: "debuff_stat", value: state.value, stat: "dmg_up_intel", duration: 1};
                state.source_skill.add_buff(actor, actor, effect2, this);
                const newState = {
                    name: "諸行無常-脱(予備)",
                    type: "special",
        ...
        if(state.name === "諸行無常-脱(予備)"){
            /**諸行無常-脱の処理*/
                actor.states.forEach(s => {
                    if (s.source_skill.name =="諸行無常" && s.stat == "dmg_up_weapon"){
                        idx = actor.states.indexOf(s);
                        if (idx > -1) actor.states.splice(idx, 1);
                    }
                })
                actor.states.forEach(s => {
                    if (s.source_skill.name =="諸行無常" && s.stat == "dmg_up_intel"){
                        idx = actor.states.indexOf(s);
                        if (idx > -1) actor.states.splice(idx, 1);
                    }
                })
        ...
        if(state.name === "母衣武者(予備)"){
            ...
            if(cnt <=5){
                ...
                const logMsg = ` -> ${state.source_skill.colored_name} の付与回数上限`;
                this.add_log(logMsg);
            }
        }
        ...
        if(state.name === "大太刀力士隊(予備)"){
            /**大太刀力士隊の処理*/
            if(Math.random() <= 0.25){
                this.process_attack_event(actor, target, 120, "weapon", state.source_skill);
            }
        }

        if(state.name === "薩摩鉄砲兵(予備)"){
            /**薩摩鉄砲兵の処理*/
            const newState = {
                name: "通常攻撃不可",
                type: "status_effect",
        ...
        if(state.name === "文武両道-文(予備)"){
            /**文武両道-文の処理*/
            let cnt = Number(state.value);
            if(cnt <5){
                const effect1 = { type: "buff_stat", value: 30, stat: "pow", duration: 99, stack_max: 5};
                state.source_skill.add_buff(actor, target, effect1, this);
                cnt += 1;
                state.value = cnt;
            }else{
        ...
        if(state.name === "文武両道-武(予備)"){
            /**文武両道-文の処理*/
            let cnt = Number(state.value);
            if(cnt <5){
                const effect1 = { type: "buff_stat", value: 30, stat: "intl", duration: 99, stack_max: 5};
                state.source_skill.add_buff(actor, target, effect1, this);
                cnt += 1;
                state.value = cnt;
            }else{
        ...
        if(state.name === "鉄砲僧兵(予備)"){
            /**鉄砲僧兵の処理*/
            const heal_turn = [1,2,5,6];
            if(heal_turn.includes(this.turn)){
                this.process_heal_event(actor, actor, 48, "ldr", state.source_skill.name);
            }
        }
...
function getHeishuFactor(casterHeishu, targetHeishu) {
    // 1. まず特定の組み合わせがあるかチェック
    const factor = HEISHU_AFFINITY[casterHeishu]?.[targetHeishu];
    if (factor !== undefined) return factor;

    // 2. 相手が「兵器」の場合など、攻撃側に関わらず適用されるルールをチェック
    if (targetHeishu === "兵器") return 1.2;
